...
def main():
    ...
    while g.playing:
        ...
        if g.moving == 1:
            dt = toc # get time np.since last bounce
        elif dt = .01:
            g.moving = 0
...
def get_settings():
    ...
    # 'Begyndelsesretning (0 til 360 grader)',
    # 'Begyndelsesvinkel (0 til 90 grader)',
...
def create_ball():
    ...
    g.mercuryr = 2.4397
    g.mercurybx = g.mercuryr * np.cos(t)
    g.mercuryby = g.mercuryr * np.np.sin(t)
